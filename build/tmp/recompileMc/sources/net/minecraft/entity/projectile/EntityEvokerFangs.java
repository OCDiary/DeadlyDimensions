...
public class EntityEvokerFangs extends Entity
{
    ...
    {
        if (this.field_190557_e == null && this.field_190558_f != null && this.worldObj instanceof WorldServer)
        {
            Entity entity = ((WorldServer)this.worldObj).getEntityFromUuid(this.field_190558_f);
    ...
    public void onUpdate()
    {
            ...
            if (this.field_190556_d)
            {
                ...
                if (this.field_190555_c == 14)
                {
                    for (int i = 0; i < 12; ++i)
                    {
                        ...
                        this.worldObj.spawnParticle(EnumParticleTypes.CRIT, d0, d1 + 1.0D, d2, d3, d4, d5, new int[0]);
                    }
                }
            }
        }
        else if (--this.field_190553_a < 0)
        {
            ...
            {
                for (EntityLivingBase entitylivingbase : this.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, this.getEntityBoundingBox().expand(0.2D, 0.0D, 0.2D)))
                {
    ...
    public void handleStatusUpdate(byte id)
    {
        ...
        if (id == 4)
        {
            ...
            if (!this.isSilent())
            {
                this.worldObj.playSound(this.posX, this.posY, this.posZ, SoundEvents.field_191242_bl, this.getSoundCategory(), 1.0F, this.rand.nextFloat() * 0.2F + 0.85F, false);
            }
        }
    }
